...
def test_LineReader_for_proper_fields_assignation(line_reader_obj,
    ...
    assert line_reader_obj.user_name == grouped_output[2]
    assert line_reader_obj.date == grouped_output[3]
    assert line_reader_obj.status_line == grouped_output[4]
    assert line_reader_obj.status_code == grouped_output[5]
    assert line_reader_obj.response_len == grouped_output[6]
    assert line_reader_obj.referer == grouped_output[7]
    assert line_reader_obj.user_agent == grouped_output[8]
    assert line_reader_obj.time == grouped_output[9]
